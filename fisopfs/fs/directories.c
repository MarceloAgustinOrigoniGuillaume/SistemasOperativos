...
static char *
splitLast(char *temp, const char *delimiter, int *ind)
{
...
static void
freeChildrenData(struct DirData *parent)
{
	if (parent == NULL) {
		return;
	}
...
freeDirData(int id)
{
	...
	prev_free->next_free =
	        resetDirData(id);  // Agrega a la lista de frees el bloque curr
}
...
void
serializeDirData(struct SerialFD *fd_out, struct DirData *dir)
{
...
void
deserializeDirData(struct SerialFD *fd_in, struct DirData *dir)
{
...
void
serializeDirs(struct SerialFD *fd_out)
{
	// No se usa el fd directamente! por tema little endian vs big endian y
	// asi Para numeros y asi esta los metodos de serial.h!
...
void
deserializeDirs(struct SerialFD *fd_in)
{
...
static struct Inode *
indexChild(struct DirData *dir, int *ind, const char *name)
{
...
char *
strndup(const char *src, int count)
{
	size_t len = count + 1;
	char *dst = malloc(len);
	if (dst) {
...
int
addChild(struct Inode *parent, struct Inode *child)
{  // Persona 2
...
struct Inode *
searchChild(struct DirData *dir, const char *name)
{
...
struct Inode *
searchRelative(const char *path)
{                                          // Persona 2
...
struct Inode *
searchNew(const char *path, char **name_child)
{  // Persona 2
...
void
readChildren(struct Inode *dir, struct DirEntries *out)
{  // Persona 2
...
char **
split(const char *str, const char *delimiter, int *count)
{

...
int
writeToBlock(struct Block *block, const char *buff, int off, const int count)
{
	if (off > block->size) {
...
static int
readFromBlock(struct Block *block, char *buff, int off, const int count)
{
	if (off > block->size) {
		return -1;  // Se fallo escritura.
	}
...
freeBlock(int id)
{
	...
	prev_free->next_free =
	        resetBlock(id);  // Agrega a la lista de frees el bloque curr
}
...
void
serializeBlockData(struct SerialFD *fd_out, struct Block *block)
{
...
void
deserializeBlockData(struct SerialFD *fd_in, struct Block *block)
{
...
void
serializeBlocks(struct SerialFD *fd_out)
{
...
void
deserializeBlocks(struct SerialFD *fd_in)
{
...
int
writeData(struct Inode *file, const char *buff, int data_off, size_t count)
{  // Persona 3
...
int
readData(struct Inode *file, char *buff_out, int data_off, size_t count)
{  // Persona 3
...
void
truncate(struct Inode *file, int size)
{  // Persona 3
	struct Block *block = getBlock(file->first_block);
	if (block == NULL) {

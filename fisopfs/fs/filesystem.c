...
int
fs_getattrs(const char *path, struct stat *st)
{
...
int
fs_truncate(const char *path, off_t new_size)
{
...
int
fs_touch(const char *path, mode_t mode, struct fuse_file_info *fi)
{
...
int
fs_mkdir(const char *path, mode_t mode  //,struct fuse_file_info *fi
)
{
...
void *
fs_init(struct fuse_conn_info *conn)
{
	...
	if (err != 0) {
		printf("Serialize open file for deserializing failed! %d "
		       "'%s'\n",
		       err,
		       filedisk);
		allocDir(root_inode);

...
void
showBytes(const uint8_t *buff, int count)
{
...
static int
tryReadAll(int fd, uint8_t *out, int count)
{
	// printf("TRY READ %d\n", count);
...
static int
tryWriteAll(int fd, const uint8_t *src, int count)
{
...
static int
writeAll(struct SerialFD *writer, const uint8_t *src, int count)
{
...
static int
readAll(int fd, uint8_t *out, int count)
{
...
int
writeInt(struct SerialFD *writer, int num)
{
	num = htonl(num);
...
int
readInt(struct SerialFD *writer, int *num)
{
...
int
writeShort(struct SerialFD *writer, short num)
{
	num = htons(num);
...
int
readShort(struct SerialFD *writer, short *num)
{
...
int
writeMsg(struct SerialFD *writer, const char *buffer, int count)
{
...
int
writeStr(struct SerialFD *writer, const char *str)
{
...
int
readStr(struct SerialFD *writer, char **out)
{
	short msg_len;
...
char *
readMsg(struct SerialFD *writer, int *ret)
{
	short msg_len;
...
int
readCapMsg(struct SerialFD *writer, char *buffer, short *msg_len, int max)
{
	int ret = readShort(writer, msg_len);
	if (ret != 0) {
		return ret;
...
struct SerialFD
openWriter(const char *filepath, int *err)
{
...
struct SerialFD
openReader(const char *filepath, int *err)
{
...
void
closeWriter(struct SerialFD *writer)
{
	close(writer->fd);
	writer->fd = -1;
}
...
struct SerialFD
openSubWriter(struct SerialFD *writerBase)
{
...
int
closeSubWriter(struct SerialFD *writerBase, struct SerialFD *writerSubitm)
{
